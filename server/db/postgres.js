...
export async function createPostgresRepos(connectionString, { log = () => {} } = {}) {
  ...
  return {
    ...
    venues: {
      async list() {
        const { rows } = await query('SELECT slug, name, menu, bot_tables FROM venues ORDER BY id')
        return normalise(rows.map((r) => ({ slug: r.slug, name: r.name, menu: r.menu, botTables: r.bot_tables })))
      },
      async upsert(venue) {
        await query(
          `INSERT INTO venues (slug, name, menu, bot_tables)
           VALUES ($1, $2, $3::jsonb, $4::integer[])
           ON CONFLICT (slug) DO UPDATE
             SET name = EXCLUDED.name, menu = EXCLUDED.menu, bot_tables = EXCLUDED.bot_tables, updated_at = now()`,
          [venue.slug, venue.name, JSON.stringify(venue.menu ?? []), venue.botTables ?? []]
        )
      }
    },
